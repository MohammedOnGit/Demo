...
// //         <p className="mt-4 text-muted-foreground text-center">Loading order details...</p>
// //       </div>
// //     );
// //   }
...
// //     if (!dateString) return "N/A";
// //     const date = new Date(dateString);
// //     return date.toLocaleDateString("en-GH", { year: "numeric", month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" });
// //   };
...
// //   const handleStatusUpdate = async (newStatus) => {
// //     const orderId = order._id || order.id;
// //     if (!orderId) return toast.error("Cannot update: Order ID not found");
...
// //   const handleSendNotification = (type) => toast.info(`Sending ${type} notification...`);

// //   const getStatusButtonConfig = (status) => {
// //     const configs = {
// //       processing: { label: "Mark as Processing", icon: PackageOpen, variant: "default", color: "bg-blue-500 hover:bg-blue-600" },
// //       confirmed: { label: "Mark as Confirmed", icon: CheckCircle, variant: "default", color: "bg-emerald-500 hover:bg-emerald-600" },
// //       shipped: { label: "Mark as Shipped", icon: Truck, variant: "default", color: "bg-purple-500 hover:bg-purple-600" },
// //       delivered: { label: "Mark as Delivered", icon: CheckCircle, variant: "default", color: "bg-green-500 hover:bg-green-600" },
// //       cancelled: { label: "Cancel Order", icon: X, variant: "destructive", color: "bg-red-500 hover:bg-red-600" },
// //     };
// //     return configs[status] || { label: `Mark as ${status}`, icon: CheckCircle, variant: "default" };
// //   };
...
// //           <div className="flex items-center gap-3 min-w-0 flex-1">
// //             {isMobile && (
// //               <Button variant="ghost" size="icon" className="h-8 w-8 -ml-2 shrink-0" onClick={onClose}>
// //                 <ChevronLeft className="h-4 w-4" />
// //               </Button>
// //             )}
// //             <div className="min-w-0 flex-1">
...
// //                   {order.orderNumber || `ORD-${order._id?.slice(-8).toUpperCase()}`}
// //                 </DialogTitle>
// //                 <Badge variant={statusConfig.variant} className={`${statusConfig.color} border px-3 py-1 font-medium flex items-center gap-1.5`}>
// //                   <StatusIcon className="h-3.5 w-3.5" />
// //                   <span className="capitalize">{statusConfig.label}</span>
// //                 </Badge>
// //               </div>
// //               <DialogDescription className="flex flex-wrap items-center gap-4 mt-2 text-sm">
// //                 <span className="flex items-center gap-1.5"><Calendar className="h-3.5 w-3.5" />{formatDate(order.orderDate)}</span>
// //                 <span className="flex items-center gap-1.5"><DollarSign className="h-3.5 w-3.5" />{formatCurrency(order.totalAmount)}</span>
// //                 <span className="flex items-center gap-1.5"><ShoppingBag className="h-3.5 w-3.5" />{order.cartItems?.length || 0} items</span>
// //               </DialogDescription>
// //             </div>
// //           </div>
// //           {!isMobile && (
...
// //               <Button variant="outline" size="sm" onClick={handlePrintInvoice} className="gap-2"><Printer className="h-4 w-4" />Print</Button>
// //               <Button variant="outline" size="sm" className="gap-2" onClick={() => handleSendNotification("email")}><Mail className="h-4 w-4" />Email</Button>
// //             </div>
// //           )}
// //         </div>
// //       </DialogHeader>
...
// //           {/* Right Column - Actions, Shipping, Payment */}
// //         </div>
// //       </div>

// //       {/* Mobile Bottom Actions */}
// //       {isMobile && (
// //         <div className="sticky bottom-0 left-0 right-0 bg-background/95 backdrop-blur-sm border-t p-4 flex items-center justify-between">
// //           <div>
// //             <p className="text-xs text-muted-foreground">Total</p>
// //             <p className="text-lg font-bold">{formatCurrency(order.totalAmount)}</p>
// //           </div>
// //           <div className="flex gap-2">
// //             <Button size="sm" variant="outline" className="gap-1.5" onClick={handlePrintInvoice}><Printer className="h-3.5 w-3.5" />Print</Button>
// //             <Button size="sm" variant="default" className="gap-1.5" onClick={() => handleStatusUpdate("processing")} disabled={isUpdating}>
...
//                     <div>
//                       <p className="font-semibold">
//                         {order.user?.userName || order.customerName || order.addressInfo?.fullName || "Customer"}
//                       </p>
//                       <p className="text-sm text-muted-foreground">
//                         {order.user?.email || order.customerEmail || "No email provided"}
//                       </p>
//                     </div>
//                   </div>
//                   <Badge variant="outline" className="gap-1.5">
...
//                     </div>
//                     <p className="text-sm">
//                       {order.addressInfo?.phone || order.addressInfo?.phoneNumber || "Not provided"}
//                     </p>
//                   </div>
...
//           </div>
//           <div className="flex gap-2">
//             <Button size="sm" variant="outline" className="gap-1.5" onClick={handlePrintInvoice}>
//               <Printer className="h-3.5 w-3.5" />
//               Print
//             </Button>
//             <Button 
...
//                 currentStatus === "processing" ? "confirmed" :
...
//                 currentStatus === "cancelled" ? "Cancelled" : "Next Status"}
//             </Button>
//           </div>
//         </div>
//       )}
...
import { toast } from "sonner";
import { updateOrderStatus, selectAdminOrderUpdating } from "@/store/admin/order-slice";
import { statusConfig, getStatusConfig, orderStatuses } from "@/config"; // Import from config

// Helper to get next possible statuses based on current status
...
// Button configuration for status updates
const getStatusButtonConfig = (status) => {
  const configs = {
    processing: { 
      label: "Mark as Processing", 
        ...
        <div className="flex items-center justify-between gap-4">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <Badge 
                  ...
                  className={`${statusConfigItem.color} ${statusConfigItem.bgColor} border px-3 py-1 font-medium flex items-center gap-1.5`}
                >
                  <StatusIcon className="h-3.5 w-3.5" />
                  <span className="capitalize">{statusConfigItem.label}</span>
                </Badge>
              </div>
              <DialogDescription className="flex flex-wrap items-center gap-4 mt-2 text-sm">
                <span className="flex items-center gap-1.5">
          ...
          {!isMobile && (
            <div className="flex items-center gap-2 shrink-0">
              ...
              <Button variant="outline" size="sm" className="gap-2" onClick={() => handleSendNotification("email")}>
                <Mail className="h-4 w-4" />
                Email
              </Button>
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-card border rounded-lg">
              ...
              <div className="p-5 space-y-3">
                ...
                {/* Status Update Buttons - Using nextStatuses from flow */}
            ...
            <div className="bg-card border rounded-lg">
              ...
              <div className="p-5 space-y-4">
                ...
                {/* Status Timeline - Using statusConfig for icons */}
                <div className="pt-4 border-t">
                  <h4 className="text-sm font-medium mb-4">Order Timeline</h4>
                  <div className="space-y-4">
                    {[
          ...
          <div className="flex gap-2">
            ...
            {nextStatuses.length > 0 && (
              ...
              >
                ...
                {isUpdating ? "Updating" : "Next Status"}
              </Button>
            )}
          </div>
        </div>
