...
function CheckAuth({ isAuthenticated, user, children, requiredRole }) {
  ...
  // Unauthenticated users: allow only specific paths
  if (!isAuthenticated) {
    const allowedUnauthPaths = [
      "/auth/login",
      "/auth/register",
      ...
      "/shop/search",
      "/shop/product/",
      "/shop/order-confirmation" // ✅ CRITICAL FIX: allow order confirmation page
    ];
  ...
  // Authenticated users: prevent access to auth pages (login/register)
  if (isAuthenticated && location.pathname.startsWith("/auth/")) {
    return user?.role === "admin" ? (
      <Navigate to="/admin/dashboard" replace />
    ) : (
  ...
  // Non-admin users: block admin routes
  if (isAuthenticated && location.pathname.startsWith("/admin/")) {
    if (user?.role !== "admin") {
      return (
        <div className="min-h-screen flex items-center justify-center bg-background p-4">
  ...
  // Admin users accessing shop pages: redirect to admin dashboard
  if (isAuthenticated && user?.role === "admin" && location.pathname.startsWith("/shop/")) {
    const allowedShopPaths = [
      "/shop/home",
      "/shop/listing",
      "/shop/search",
      "/shop/product/",
      "/shop/order-confirmation" // ✅ Also allow admin to view order confirmation if needed
    ];
    const isAllowed = allowedShopPaths.some(path => 
      location.pathname.startsWith(path)
    );
