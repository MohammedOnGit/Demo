...
function UserCartWrapper({ cartItems = [], setOpenCartSheet }) {
  ...
  const { hasItems, totalItems, subtotal, shippingCost, finalTotal, remainingForFreeShipping } = useMemo(() => {
    const subtotal = cartItems.reduce((sum, item) => {
      const price = item.salePrice > 0 ? Number(item.salePrice) : Number(item.price || 0);
      const quantity = Number(item.quantity) || 1;
      return sum + price * quantity;
    ...
    return {
      ...
      shippingCost,
      finalTotal: subtotal + shippingCost,
      remainingForFreeShipping: Math.max(SHIPPING_THRESHOLD - subtotal, 0),
    };
  }, [cartItems]);
  ...
  const checkout = useCallback(() => {
    ...
    setTimeout(() => {
      setOpenCartSheet?.(false);
      navigate("/shop/checkout", {
        state: { cartItems, subtotal, shippingCost, totalAmount: finalTotal },
      });
    }, ANIMATION_DURATION);
  }, [navigate, setOpenCartSheet, cartItems, subtotal, shippingCost, finalTotal]);
    ...
    >
      ...
      >
        {hasItems ? (
          ...
          <div className="h-full flex flex-col items-center justify-center text-center p-6">
            ...
            <Button variant="outline" className="gap-2" onClick={navigateToProducts}>
              <ShoppingCart className="h-4 w-4" />
              Browse Products
            </Button>
          </div>
      ...
      {hasItems && (
        <div className="border-t px-4 py-4 space-y-4 flex-shrink-0">
          ...
          <Button variant="outline" className="w-full h-11" onClick={navigateToCart}>
            View Full Cart Details
          </Button>
        </div>
      )}
