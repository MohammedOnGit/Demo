...
function ProductFilter({ filters = {}, handleFilter, onClearFilters }) {
  ...
  const isExpanded = useCallback((key) => expandedSections[key] !== false, [expandedSections]);
  ...
  const FilterContent = useMemo(() => {
    return function FilterContentComponent() {
      return (
        <div className="space-y-6">
          <div className="flex items-center justify-between">
            <h3 className="text-lg font-semibold">Filters</h3>
                        ...
                        return (
                          ...
                          >
                            <Checkbox
                              id={`${key}-${index}`}
                              checked={checked}
                              onCheckedChange={() => handleCheckboxChange(key, id)}
                            />
                            <label
                              htmlFor={`${key}-${index}`}
                              className={cn(
  ...
  }, [filters, activeFilterCount, onClearFilters, toggleSection, isExpanded, handleCheckboxChange]);
