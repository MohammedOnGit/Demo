...
const ShoppingProductTile = ({
  ...
  const cartItem = useMemo(() => 
    cartItems.find((item) => item.productId === _id || item._id === _id)
  , [cartItems, _id]);

  const isInWishlist = useMemo(() => 
    wishlistItems.some((item) => item.productId === _id || item.product?._id === _id)
  , [wishlistItems, _id]);
  ...
  }, [product, stockInfo, isInCart, cartQuantity, handleAddtoCart]);

  const handleViewDetailsClick = useCallback((e) => {
    e.stopPropagation();
    e.preventDefault();
    handleGetProductDetails(_id);
  }, [_id, handleGetProductDetails]);
  ...
  }, [dispatch, user?.id, _id, isInWishlist]);
  ...
  return (
    <Card className="group relative overflow-hidden hover:shadow-lg transition">
      ...
      <CardHeader className="p-0 relative">
        ...
        {/* Quick Action Buttons - Now with higher z-index and pointer-events-auto */}
