...
const CATEGORIES = ["Men's Fragrance", "Women's Perfume", "Unisex", "Luxury", "Gift Sets"];
const SORT_OPTIONS = {
  relevance: "Relevance",
...
export default function SearchPage() {
  ...
  const hasActiveFilters = useMemo(() => 
    filters.category.length > 0 || filters.priceRange.max < 1000 || filters.inStock
  , [filters]);
  ...
  const handleCategoryChange = useCallback((cat, checked) => {
    setFilters(prev => ({
      ...prev,
      category: checked 
        ? [...prev.category, cat]
  ...
  const getSortLabel = useCallback((value) => {
    return SORT_OPTIONS[value] || "Relevance";
  }, []);
