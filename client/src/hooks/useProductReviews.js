...
export function useProductReviews(productId) {
  ...
  const submitReview = useCallback(async (reviewData) => {
    ...
    if (reviewData.reviewMessage.trim().length < MIN_REVIEW_LENGTH) {
  ...
  const getPurchaseStatusText = useCallback(() => {
    switch (purchaseStatus) {
      case 'eligible':
        return { text: 'You can review this product', type: 'success' };
      case 'not-purchased':
        return { text: 'You can only review purchased products', type: 'warning' };
      case 'already-reviewed':
        return { text: 'You have already reviewed this product', type: 'info' };
      case 'not-logged-in':
        return { text: 'Please login to submit a review', type: 'info' };
      case 'error':
        return { text: 'Error checking eligibility', type: 'error' };
      default:
        return { text: 'Checking eligibility...', type: 'info' };
    }
  }, [purchaseStatus]);
