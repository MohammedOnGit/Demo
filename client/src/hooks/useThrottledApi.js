...
export function useThrottledApi(apiCall, dependencies = [], options = {}) {
  ...
  }, [apiCall, cacheKey, onSuccess, onError, retryAttempt, retryCount, retryDelay, cleanup]);
  ...
  const executeApiCall = useCallback(async (force = false, isRetry = false) => {
    ...
    makeRequest(force);
  }, [enabled, cacheKey, throttleMs, onSuccess, cleanup, makeRequest]);
  ...
  useEffect(() => {
    ...
    return cleanup;
  }, [executeApiCall, cleanup, enabled, ...dependencies]);
