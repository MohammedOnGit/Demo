...
const Wishlist = () => {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible' && user && !isFetchingRef.current) {
        const lastFetch = Number(localStorage.getItem('lastWishlistFetch')) || 0;
        const shouldRefresh = Date.now() - lastFetch > REFRESH_INTERVAL;
        if (shouldRefresh) {
          fetchWishlistData();
        }
      }
  ...
  const handleMoveToCart = useCallback(async (productId) => {
    try {
  ...
  }, [dispatch, selectedItems, syncCart]);
  ...
  const getStockStatus = useCallback((stock) => {
    ...
    if (stock <= LOW_STOCK_THRESHOLD) return { text: "Low Stock", color: "warning" };
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-linear-to-b from-background to-muted/10 flex items-center justify-center">
                ...
                <Card key={wishlistItem._id || productId} className="border-2 rounded-2xl">
                  <CardContent className="p-6">
                    <div className="flex flex-col sm:flex-row gap-6">
                      <div className="sm:w-32 sm:h-32 shrink-0">
                        <img
                          src={product?.image || "/placeholder.jpg"}
                          alt={product?.title}
