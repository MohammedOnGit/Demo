...
function ShoppingCheckout() {
  ...
  const validateCheckout = useCallback(() => {
    ...
    return true;
  }, [selectedAddress, cartItems, user, hasStockIssues, navigate]);
    ...
    try {
      const formattedCartItems = cartItems.map((item) => {
        ...
        return {
          ...
          salePrice: item.salePrice > 0 ? Number(Number(item.salePrice).toFixed(2)) : undefined,
          quantity,
      ...
      const message = typeof error?.message === "string" ? error.message : "Checkout failed";
  ...
  }, [cartItems, user, selectedAddress, totalCartAmount, dispatch, navigate, validateCheckout]);

  const isProcessingState = isProcessing || isOrderCreating;
  const canCheckout = Boolean(selectedAddress) &&
    cartItems.length > 0 &&
    Boolean(user?.id) &&
    Boolean(user?.email) &&
    !hasStockIssues;
