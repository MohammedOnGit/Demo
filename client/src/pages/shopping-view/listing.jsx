...
const STORAGE_KEYS = {
  ...
  SORT: "shop-sort",
  INVENTORY_FILTER: "shop-inventory-filter",
};
...
export default function ShoppingListing() {
  ...
  useEffect(() => {
    ...
    setFilters(
      Object.keys(urlFilters).length
        ? urlFilters
        : JSON.parse(sessionStorage.getItem(STORAGE_KEYS.FILTERS) || "{}")
    );

    setSort(sessionStorage.getItem(STORAGE_KEYS.SORT) || "price-lowtohigh");
    setInventoryFilter(sessionStorage.getItem(STORAGE_KEYS.INVENTORY_FILTER) || "all");
  }, [searchParams]);
  ...
  const hasFilters = Object.keys(filters).length > 0 || inventoryFilter !== "all";
