...
const PRICE_RANGE_DEFAULT = [0, 1000];
const PAGE_SIZE = 20;
...
const SearchPage = () => {
  ...
  const handleSearch = useCallback((e) => {
    ...
    if (filters.brand?.length) params.set('brand', filters.brand[0]);
    params.set('page', '1');
  ...
  }, [dispatch, localQuery, setSearchParams]);
  ...
  const handlePageChange = useCallback((newPage) => {
    ...
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, [dispatch, searchParams, localQuery, filters, setSearchParams]);

  const categories = useMemo(() => 
    [...new Set(searchResults.map(p => p.category))], [searchResults]
  );
  
  const brands = useMemo(() => 
    [...new Set(searchResults.map(p => p.brand).filter(Boolean))], [searchResults]
  );
  ...
  const getSortLabel = useCallback((sort) => {
    switch (sort) {
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-4">
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" className="gap-3 h-11 px-4 rounded-xl border-2">
                    <span className="font-medium">Sort by:</span> {getSortLabel(sortBy)}
                    <ChevronDown className="h-4 w-4 ml-1" />
                  </Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end" className="w-56 rounded-xl shadow-lg border-2">
