...
import { createSlice, createAsyncThunk } from "@reduxjs/toolkit";
import axiosInstance from "../../../utils/axios-interceptor"; // ✅ your interceptor
...
// Helper – uses axiosInstance (no auth needed for product lookup)
const checkProductStock = async (productId, requestedQuantity) => {
  try {
    const response = await axiosInstance.get(`/shop/products/get/${productId}`);
    const product = response.data?.data || response.data;
    if (!product) return { available: false, reason: "Product not found" };
    if (product.isActive === false) return { available: false, reason: "Product is not available" };
    const availableStock = product.availableStock || Math.max(0, (product.totalStock || 0) - (product.reservedStock || 0));
    if (product.allowBackorders) {
      return { 
        available: true, 
        availableStock,
    ...
    console.error("Stock check error:", error);
    return { available: false, reason: "Failed to check stock availability" };
  }
};
...
export const fetchCartItems = createAsyncThunk(
  ...
  async (userId, { rejectWithValue }) => {
    try {
      ...
      if (response.data.success) {
        const stockSummary = response.data.stockSummary || initialState.stockSummary;
        const validatedItems = [];
        for (const item of response.data.data?.items || []) {
          if (item.productId) {
            const stockCheck = await checkProductStock(item.productId, item.quantity);
...
export const addToCart = createAsyncThunk(
  ...
  async ({ userId, productId, quantity = 1 }, { rejectWithValue, getState }) => {
    try {
      ...
      if (error.response?.status === 401) {
        // Optimistic fallback – use shopCart state
        const state = getState();
        const product = state.shopProducts?.products?.find(p => p._id === productId) || {};
        const availableStock = product.availableStock || product.totalStock || 0;
        const currentCartQty = state.shopCart?.items?.find(i => i.productId === productId)?.quantity || 0;
        if (availableStock < (currentCartQty + quantity) && !product.allowBackorders) {
          return rejectWithValue({
            message: `Only ${availableStock} item(s) available`,
            availableStock,
        ...
        const updatedItems = [...(state.shopCart?.items || []), newItem];
        return {
          items: updatedItems,
          cartCount: (state.shopCart?.cartCount || 0) + quantity,
          subtotal: (state.shopCart?.subtotal || 0) + (product.salePrice || product.price || 0) * quantity,
          message: "Added to cart (offline mode)"
        };
      }
      if (error.response?.data?.message?.includes('stock')) {
...
export const updateCartQuantity = createAsyncThunk(
  ...
  async ({ userId, productId, quantity }, { rejectWithValue, getState }) => {
    try {
      const response = await axiosInstance.put('/shop/cart/update', { userId, productId, quantity });
      if (response.data.success) {
        return {
          ...response.data.data,
          stockChange: response.data.stockChange
        ...
        const errorData = response.data;
        if (errorData.message?.includes('stock') || errorData.message?.includes('available')) {
          return rejectWithValue({
            message: errorData.message,
            availableStock: errorData.availableStock,
            maxAllowed: errorData.maxAllowed,
...
export const deleteCartItem = createAsyncThunk(
  ...
  async ({ userId, productId }, { rejectWithValue, getState }) => {
    try {
      ...
      if (error.response?.status === 401) {
        const state = getState();
        const item = state.shopCart?.items?.find(i => i.productId === productId);
        if (item) {
          return {
            productId,
            items: state.shopCart.items.filter(i => i.productId !== productId),
            cartCount: state.shopCart.cartCount - item.quantity,
            subtotal: state.shopCart.subtotal - (item.salePrice || item.price || 0) * item.quantity
          };
        }
      }
      return rejectWithValue(error.response?.data?.message || "Failed to delete item");
...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    updateCartCount: (state, action) => { state.cartCount = action.payload; },
    updateCartSubtotal: (state, action) => { state.subtotal = action.payload; },
    clearCartError: (state) => { state.error = null; },
    setCartStockError: (state, action) => { state.error = action.payload; },
    updateStockSummary: (state, action) => {
      state.stockSummary = { ...state.stockSummary, ...action.payload };
    }
  },
  extraReducers: (builder) => {
    builder
      .addCase(fetchCartItems.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(fetchCartItems.fulfilled, (state, action) => {
        state.isLoading = false;
        state.items = action.payload.items || [];
        state.cartCount = state.items.reduce((sum, item) => sum + item.quantity, 0);
      ...
      .addCase(fetchCartItems.rejected, (state, action) => { state.isLoading = false; state.error = action.payload; })
      ...
      .addCase(updateCartQuantity.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(updateCartQuantity.fulfilled, (state, action) => {
        state.isLoading = false;
        state.items = action.payload.items || [];
        state.cartCount = state.items.reduce((sum, item) => sum + item.quantity, 0);
      ...
      .addCase(updateCartQuantity.rejected, (state, action) => { state.isLoading = false; state.error = action.payload; })
      .addCase(deleteCartItem.pending, (state) => { state.isLoading = true; state.error = null; })
      .addCase(deleteCartItem.fulfilled, (state, action) => {
        state.isLoading = false;
        if (action.payload.items) {
          state.items = action.payload.items;
      ...
      .addCase(deleteCartItem.rejected, (state, action) => { state.isLoading = false; state.error = action.payload; });
  }
});
